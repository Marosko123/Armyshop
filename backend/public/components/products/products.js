...
function getCategoryFromSubcategory(subcategory) {
    ...
    for (const category in categorySubcategoryMap) {
        if (
            categorySubcategoryMap[category].includes(
                subcategoriesDict[subcategory]
            )
        ) {
            return category;
        }
...
function getSubcategoriesForCategory() {
    ...
    if (isCategory) {
        ...
        subcategories = subcategories.map(
            (subcategory) => reverseSubcategoriesDict[subcategory]
        );

        // Generate the HTML markup for the subcategories
        const subcategoriesMarkup = subcategories
            .map(
                (subcategory) => `
...
function getProductsHTML(products, notLikedImg, likedImg, likedArray) {
    ...
    for (const product of products) {
        ...
        productsHTML += `
        <div class="card m-3" data-product-id="${
            product.id
            ...
            <div class="card-body d-flex align-items-center justify-content-between mx-auto">
                <div class="mt-2">
                    ...
                    <p class="card-text">${formatPriceMillions(
                        product.price
                    )} €</p>
                </div>
                <!-- image source: flaticon.com -->
                <img src="${likedVersion}" alt="" width="14%" class="liked-photo ${
            isLiked ? "liked-photo-new" : ""
    ...
    for (const card of productCards) {
        ...
        cartButton.addEventListener("click", async () => {
            ...
            previousCount = parseInt(
                data[productId] ? data[productId].count : 0
            );
            // console.log(productId);

            filteredProduct = Object.values(GlobalVariables.products).filter(
                (globalProduct) => globalProduct.id == productId
            )[0];
            ...
            if (user)
                await ServerRequester.postToUrl(
                    `/baskets/add/${user.id}/${productId}/${data[productId].count}`,
                    {
                        body: data,
                    }
                );
...
function filterByLicense(products) {
    ...
    if (licenseNeeded && licenseNeeded != "0") {
        ...
        // don't include the products that need license
...
function toggleCheckmark() {
    ...
    if (checkmark.classList.contains("checkmark")) {
        ...
        checkmark.src =
            "http://127.0.0.1:8000/images/productDetailImages/checkmark.png";
    }
}
